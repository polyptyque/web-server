...
# docker run --link gramme2-mysql:mysql -e VIRTUAL_HOST=polyptyque.photo --name polyptyque.photo --restart=always -v $(pwd)/uploads:/usr/src/app/uploads -v $(pwd)/mixes:/usr/src/app/mixes polyptyque:latest
